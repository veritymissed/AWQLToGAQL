...
const fs = require('fs');
var mapping_table = JSON.parse(fs.readFileSync('./mapping_table.json'));
...
var AWQLQB = function () {
  ...
  _createClass(AWQLQB, [{
    ...
    value: function asGAQL() {
      ...
      if (fields) fieldsString += fields.join(' , ');
      if (timeRange) timeRangeString += _moment(this.query.timeRange.since).format("YYYY-MM-DD") + ', ' + _moment(this.query.timeRange.until).format("YYYY-MM-DD");
      var filterings = this.filterRules.map(function (rule) {
      ...
      if (level) queryString += 'FROM ' + level + ' ';
      if (filteringsString) queryString += 'WHERE ' + filteringsString + ' ';
      if (timeRangeString) queryString += 'BETWEEN ' + timeRangeString + ' ';
