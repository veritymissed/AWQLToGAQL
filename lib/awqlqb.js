...
const fs = require('fs');
var mapping_table = require('../mapping_table.json');
...
var AWQLQB = function () {
  ...
  _createClass(AWQLQB, [{
    ...
    key: 'asGAQL',
    value: function asGAQL() {
      var _query = this.query,
          level = _query.level,
          fields = _query.fields,
          timeRange = _query.timeRange;
      fields = fields.map((val) => mapping_table[val]);
      level = mapping_table[level];
      ...
      if (fields) fieldsString += fields.join(' , ');
      if (timeRange) timeRangeString += `'` + _moment(this.query.timeRange.since).format("YYYY-MM-DD") + `' AND '` + _moment(this.query.timeRange.until).format("YYYY-MM-DD") + `'`;
      var filterings = this.filterRules.map(function (rule) {
        var ruleString = mapping_table[rule.field] + ' ' + rule.operator + ' ';
        ruleString += typeof rule.values === 'string' ? rule.values : '(' + rule.values.join(' , ') + ')';
        return ruleString;
      });
      filteringsString = filterings.join(' AND ');
      ...
      if (timeRangeString){
        if (filteringsString){
          queryString += ` AND segments.date BETWEEN ` + timeRangeString + ` `;
        }
        else{
          queryString += ` WHERE segments.date BETWEEN ` + timeRangeString + ` `;
        }
      }
